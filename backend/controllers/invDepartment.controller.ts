...
export const getInvDepartment = (req: Request, res: Response) => {
  if (departments.length <= 0) {
    return res
      .status(500)
      .json({ success: false, message: "Missing Department Data " });
  }

  res.status(200).json({ success: true, data: {departments,maintenanceRequests} });
};
