...
const ExpensesAnalytics: React.FC<Props> = ({
  expenses,
  staffFromPayroll = 0,
}) => {
  ...
  const categoryColors: Record<
    Exclude<Expense["category"], "other">,
    string
  > = {
  ...
  const categoryOrder = useMemo<Array<Exclude<Expense["category"], "other">>>(
    () => [
      "utilities",
      "supplies",
      "maintenance",
  ...
  const chartData = (useDemo ? demo.chartData : actual.chartData) as Array<{
    category: string;
    amount: number;
    rawCat: Expense["category"];
  }>;
  const categoriesPresent = (
    useDemo ? demo.categoriesPresent : actual.categoriesPresent
  ) as Expense["category"][];
            ...
            <div className="flex items-center justify-center w-full h-[320px] border border-heritage-light/60 rounded-2xl bg-white/70">
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500">
                  Data will appear once you add expenses.
                </p>
              </div>
            </div>
          ) : (
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-2xl shadow-xl p-3 min-w-[220px]">
                          ...
                          <div className="text-lg font-black text-heritage-green">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 pb-2 mt-3 text-xs text-gray-600">
              {categoriesPresent.map((cat) => {
                ...
                return (
                  <div
                    key={cat}
                    className="flex items-center gap-2 px-2 py-1 bg-white border rounded-full shadow-sm border-gray-200/80"
                  >
                    <span
                      ...
                      style={{
                        backgroundColor:
                          categoryColors[
                            cat as Exclude<Expense["category"], "other">
                          ],
                      }}
                    />
