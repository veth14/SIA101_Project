...
export const getDailyRevenueData = () => {
    return [
...
export const getYearlyRevenueData = () => {
    return [
...
export const getRevenueData = (timeframe) => {
    switch (timeframe) {
        case 'weekly':
...
export const calculateChartMetrics = (data) => {
    ...
    const bestPeriod = data.reduce((best, current) => current.revenue > best.revenue ? current : best, data[0] || { day: '', revenue: 0 });
...
export const generateChartCoordinates = (data, maxValue, viewBoxWidth = 400, viewBoxHeight = 280) => {
...
export const generateSVGPath = (coordinates, closePath = false) => {
    if (coordinates.length === 0)
        return '';
    const pathData = coordinates.map((coord, index) => index === 0 ? `M ${coord.x},${coord.y}` : `L ${coord.x},${coord.y}`).join(' ');
