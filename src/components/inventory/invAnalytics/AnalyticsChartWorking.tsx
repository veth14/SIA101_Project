...
import { subscribeToInventoryItems, type InventoryItem } from '../invItems/items-backendLogic/inventoryService';
...
// Tooltip typing aligned with RevenueTrends
        ...
        {payload.map((entry: TooltipPayload, index: number) => (
          <div key={`tooltip-${String(index)}`} className="flex items-center justify-between gap-4 mb-1">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-gray-600">Inventory stock value</span>
            </div>
...
interface InventoryPoint {
  ...
  amount: number; // total inventory value represented at that date
}
...
const buildInventoryData = (
  ...
  const result: InventoryDataPoint[] = [];
...
export const AnalyticsChart: React.FC = () => {
  ...
  const inventoryPoints: InventoryPoint[] = useMemo(() => {
    if (!items.length) return [];

    return items.map((item) => {
      const value = (item.currentStock || 0) * (item.unitPrice || 0);
      const dateStr = item.lastRestocked || item.updatedAt?.toISOString() || item.createdAt?.toISOString() || new Date().toISOString();
      return {
        date: dateStr,
        amount: value,
      };
  ...
  const inventoryData = useMemo(
    () => buildInventoryData(inventoryPoints, activeTimeframe),
    [inventoryPoints, activeTimeframe]
  );

  const metrics = useMemo(() => calculateChartMetrics(inventoryData), [inventoryData]);
    ...
    <div className="relative overflow-hidden shadow-2xl bg-white/95 backdrop-blur-2xl rounded-3xl border-white/60 animate-fade-in">
      ...
      <div className="relative z-10">
            ...
            <div className="flex items-center space-x-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-sm font-semibold text-gray-600">Stock value based on restock dates</p>
        ...
        <div className="px-4 py-6">
          ...
          <div className="flex items-center justify-center pb-2 mt-2">
            <div className="flex items-center gap-2 text-xs text-gray-600">
              ...
              <span>
                {activeTimeframe === 'monthly'
                  ? 'Weekly inventory stock value (current month, based on last restock)'
                  : 'Monthly inventory stock value (current year, based on last restock)'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 px-8 py-6 sm:grid-cols-2 md:grid-cols-4">
          ...
          <div className="p-4 border shadow-sm bg-white/80 rounded-xl border-heritage-light">
            <div className="text-sm font-medium text-gray-500">Average Bucket Value</div>
            <div className="text-2xl font-bold text-heritage-green">
          ...
          <div className="p-4 border shadow-sm bg-white/80 rounded-xl border-heritage-light">
            <div className="text-sm font-medium text-gray-500">Highest Bucket</div>
            <div className="text-2xl font-bold text-heritage-green">
