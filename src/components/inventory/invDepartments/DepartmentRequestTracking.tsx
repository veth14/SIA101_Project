...
export const UltraPremiumRequestTable: React.FC<UltraPremiumRequestTableProps> = ({
  searchTerm,
  onSearchChange,
  selectedDepartment,
  onDepartmentChange,
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter(request => {
      ...
      const matchesSearch = searchLower === '' || 
        ...
        (request.itemService && request.itemService.toLowerCase().includes(searchLower)) ||
        (request.requestedBy && request.requestedBy.toLowerCase().includes(searchLower)) ||
      ...
      <div className="px-8 py-6 bg-gradient-to-r from-slate-50 to-white border-b border-gray-200/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-xl font-black text-gray-900">Maintenance Request Tracking</h3>
              <p className="text-sm text-gray-500 font-medium">
                Showing {startIndex + 1}-{Math.min(endIndex, filteredRequests.length)} of {filteredRequests.length} requests • Page {currentPage} of {totalPages}
                {searchTerm && <span className="ml-2 text-heritage-green">• Searching: "{searchTerm}"</span>}
                {selectedDepartment !== 'All Departments' && <span className="ml-2 text-blue-600">• Dept: {selectedDepartment}</span>}
              </p>
            </div>
          </div>
        ...
        <table className="w-full h-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {currentRequests.map((request) => (
              <tr key={request.id} className="hover:bg-gray-50 transition-colors duration-200 h-16">
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm font-medium text-gray-900 font-mono">{request.id}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4">
                  <div className="max-w-xs">
                    <p className="text-sm font-medium text-gray-900 truncate">{request.itemService}</p>
                    <p className="text-xs text-gray-500 mt-1">Service Request</p>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-emerald-700">{request.requestedBy.split(' ').map((n: string) => n[0]).join('')}</span>
                    </div>
            ...
            {Array.from({ length: Math.max(0, itemsPerPage - currentRequests.length) }).map((_, index) => (
