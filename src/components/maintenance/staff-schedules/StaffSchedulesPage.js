...
// Import utilities
import { getDayFromDate, formatDateForDisplay, getWeekNumber, getWeekDateRange, filterSchedulesByWeek, getDepartment, getShiftTime } from './utils';
// Import components
...
const StaffSchedulesPage = () => {
    ...
    const [selectedShift, setSelectedShift] = useState('7am-3pm');
    ...
    const uniqueClassifications = Array.from(new Set(staffList.map(staff => staff.classification).filter((c) => Boolean(c)))).sort();
    const filteredWeeklySchedule = Object.fromEntries(Object.entries(weeklySchedule).filter(([_, data]) => {
        ...
        if (selectedClassification !== 'all') {
            if (data.classification !== selectedClassification)
                return false;
        }
        return true;
    }));
    ...
    const organizeWeeklySchedule = useCallback((schedulesData) => {
        const organized = {};
    ...
    useEffect(() => {
        const weekFiltered = filterSchedulesByWeek(schedules, currentWeekRange.start, currentWeekRange.end);
    ...
    const handleStaffToggle = (staffId) => {
        setSelectedStaff(prev => prev.includes(staffId)
    ...
    return (_jsxs("div", { className: "min-h-screen bg-[#F9F6EE]", children: [_jsxs("div", { className: "fixed inset-0 pointer-events-none", children: [_jsx("div", { className: "absolute top-10 left-10 w-96 h-96 bg-gradient-to-r from-heritage-green/5 to-emerald-100/20 rounded-full blur-3xl animate-pulse opacity-30" }), _jsx("div", { className: "absolute top-32 right-16 w-80 h-80 bg-gradient-to-r from-blue-100/20 to-indigo-100/20 rounded-full blur-3xl animate-pulse delay-1000 opacity-25" }), _jsx("div", { className: "absolute bottom-16 left-1/4 w-72 h-72 bg-gradient-to-r from-heritage-light/10 to-heritage-neutral/10 rounded-full blur-3xl animate-pulse delay-2000 opacity-20" }), _jsx("div", { className: "absolute inset-0 opacity-5", children: _jsx("div", { className: "absolute inset-0", style: {
                            ...
                            } }) })] }), _jsxs("div", { className: "relative z-10 px-2 sm:px-4 lg:px-6 py-4 space-y-6 w-full", children: [_jsx(ScheduleHeader, {}), _jsx(ScheduleFilters, { onCreateSchedule: () => setIsModalOpen(true), currentWeekOffset: currentWeekOffset, setCurrentWeekOffset: setCurrentWeekOffset, currentWeekRange: currentWeekRange, selectedDepartment: selectedDepartment, setSelectedDepartment: setSelectedDepartment, selectedClassification: selectedClassification, setSelectedClassification: setSelectedClassification, uniqueClassifications: uniqueClassifications }), _jsx(ScheduleTable, { weeklySchedule: filteredWeeklySchedule, selectedDepartment: selectedDepartment, selectedClassification: selectedClassification }), _jsx(CreateScheduleModal, { isOpen: isModalOpen, onClose: () => setIsModalOpen(false), selectedDate: selectedDate, setSelectedDate: setSelectedDate, selectedShift: selectedShift, setSelectedShift: setSelectedShift, selectedStaff: selectedStaff, handleStaffToggle: handleStaffToggle, staffList: staffList, loading: loading, onSubmit: handleSubmit })] })] }));
};
