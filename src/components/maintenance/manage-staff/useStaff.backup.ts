...
export function useStaff() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      // Map attendance records by staffId with active status (timeIn exists and timeOut missing)
      const attendanceMap = new Map<string, boolean>();
      attendanceSnapshot.forEach(doc => {
        const data = doc.data();
        if (data.staffId) {
          // If no timeOut, staff is active
          const isActive = !data.timeOut;
          // If multiple records, prioritize active true if any
      ...
      querySnapshot.forEach((doc) => {
        ...
        staffData.push({
          ...
          // Add isActive based on attendanceMap
          isActive: attendanceMap.get(doc.id) || false,
        });
      })
