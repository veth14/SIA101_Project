...
import { collection, getDocs, query, limit as limitQuery, onSnapshot, doc, runTransaction, orderBy, startAfter } from 'firebase/firestore';
...
export async function startListeners(onFoundUpdate, onLostUpdate, pageSize = 20) {
...
export function stopListeners() {
    if (unsubFound) {
        try {
            ...
            console.warn('unsubFound error', err);
        }
        unsubFound = null;
    }
    if (unsubLost) {
        try {
            ...
            console.warn('unsubLost error', err);
        }
        unsubLost = null;
    }
    isListening = false;
}
export async function fetchPage(collectionName, pageSize = 10, startAfterDoc) {
    ...
    const items = snap.docs.map(d => mapDocToItem(d));
    const map = {};
    snap.docs.forEach(d => {
        const it = mapDocToItem(d);
        map[it.id] = d.id;
...
export const allocateSequentialId = async (collectionName) => {
...
export const createAndReserveDoc = async (collectionName, payload, maxRetries = 3) => {
    ...
    const counterRef = (name) => doc(db, 'lf_sequence', name);
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const msg = (err && typeof err === 'object' && err !== null && 'message' in err) ? String(err.message ?? '') : '';
            if (msg === 'DOC_EXISTS') {
