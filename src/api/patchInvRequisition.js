...
const usePatchInvRequisition = () => {
    const [loadingForPatchInvRequisition, setLoadingForPatchInvRequisition] = useState(false);
    const patchInvRequisition = async (id, dataToUpdate) => {
        try {
            setLoadingForPatchInvRequisition(true);
            // router.patch("/update-procurement-order/:id", ...)
            const response = await axios.patch(`/inventory-requisition/patch-requisition/${id}`, dataToUpdate);
