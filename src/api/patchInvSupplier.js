...
const usePatchInvSupplier = () => {
    const [loadingForPatchInvSupplier, setLoadingForPatchInvSupplier] = useState(false);
    const patchInvSupplier = async (id, dataToUpdate) => {
        try {
            setLoadingForPatchInvSupplier(true);
            // router.patch("/update-procurement-order/:id", ...)
            const response = await axios.patch(`/inventory-supplier/patch-supplier/${id}`, dataToUpdate);
