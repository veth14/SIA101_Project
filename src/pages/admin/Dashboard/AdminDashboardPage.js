...
export const AdminDashboardPage = () => {
                ...
                const applyCountsIfNeeded = async () => {
                    try {
                        if (needRoomCounts) {
                            ...
                            const availableRoomsSnap = await getCountFromServer(query(collection(db, 'rooms'), where('status', '==', 'available')));
                        ...
                        if (currentGuests === null) {
                            const checkedInSnap = await getCountFromServer(query(collection(db, 'bookings'), where('status', '==', 'checked-in')));
                            currentGuests = Number(checkedInSnap.data().count || 0);
                        }
                        if (!arrivalsHasToday) {
                            try {
                                const arrivalsSnap = await getCountFromServer(query(collection(db, 'bookings'), where('checkIn', '==', todayKey)));
    ...
    return (_jsxs("div", { className: "min-h-screen bg-[#F9F6EE]", children: [_jsxs("div", { className: "fixed inset-0 pointer-events-none", children: [_jsx("div", { className: "absolute rounded-full top-10 left-10 w-96 h-96 bg-gradient-to-r from-heritage-green/5 to-emerald-100/20 blur-3xl animate-pulse opacity-30" }), _jsx("div", { className: "absolute rounded-full opacity-25 bottom-16 right-16 w-80 h-80 bg-gradient-to-r from-blue-100/20 to-indigo-100/20 blur-3xl animate-pulse" })] }), _jsxs("div", { className: "relative z-10 w-full px-4 py-6 space-y-6 lg:px-8", children: [_jsx(AdminDashboardStats, { stats: stats }), _jsxs("div", { className: "grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8", children: [_jsx(RevenueTrendsCard, { bookings: bookingsData }), _jsx(SmartInsightsCard, {})] })] })] }));
};
