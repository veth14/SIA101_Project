 ...
 */
async function updateStats(updates: Record<string, any>) {
  try {
 ...
 */
export async function updateBookingCount(delta: number, checkInDate: string) {
 ...
 */
export async function updateRevenue(delta: number) {
  await updateStats({
 ...
 */
export async function updateArrivals(delta: number, arrivalDate: string) {
  await updateStats({
 ...
 */
export async function updateRoomCounts(totalRoomsDelta: number, availableRoomsDelta: number) {
  const updates: Record<string, any> = {};
  if (totalRoomsDelta !== 0) updates.totalRooms = increment(totalRoomsDelta);
  if (availableRoomsDelta !== 0) updates.availableRooms = increment(availableRoomsDelta);
 ...
 */
export async function updateStaffCounts(totalStaffDelta: number, activeStaffDelta: number) {
  const updates: Record<string, any> = {};
  if (totalStaffDelta !== 0) updates.totalStaff = increment(totalStaffDelta);
  if (activeStaffDelta !== 0) updates.activeStaff = increment(activeStaffDelta);
  if (Object.keys(updates).length > 0) {
    await updateStats(updates);
  }
}
 ...
 */
export async function updateCurrentGuests(delta: number) {
  await updateStats({
